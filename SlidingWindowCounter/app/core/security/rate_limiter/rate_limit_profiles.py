...
# API_ACCOUNT_LIMITER = SlidingWindowCounter(window_size=60.0, limit=1000)

# Public Endpoint Limits
# PUBLIC_IP_LIMITER = SlidingWindowCounter(window_size=60.0, limit=30)

# Admin Endpoint Limits (more permissive)
# ADMIN_IP_LIMITER = SlidingWindowCounter(window_size=60.0, limit=200)

# High-Security Endpoints (very restrictive)
# SECURE_IP_LIMITER = SlidingWindowCounter(window_size=300.0, limit=5)
# SECURE_ACCOUNT_LIMITER = SlidingWindowCounter(window_size=300.0, limit=5)
...
class RateLimitPresets:
    ...
    @staticmethod
    def per_minute(limit: int) -> SlidingWindowCounter:
        """Create a rate limiter with per-minute window."""
        return SlidingWindowCounter(window_size=WindowSizes.MINUTE_1, limit=limit)

    @staticmethod
    def per_second(limit: int) -> SlidingWindowCounter:
        """Create a rate limiter with per-second window."""
        return SlidingWindowCounter(window_size=WindowSizes.SECONDS_1, limit=limit)

    @staticmethod
    def per_hour(limit: int) -> SlidingWindowCounter:
        """Create a rate limiter with per-hour window."""
        return SlidingWindowCounter(window_size=WindowSizes.HOUR_1, limit=limit)

    @staticmethod
    def per_day(limit: int) -> SlidingWindowCounter:
        """Create a rate limiter with per-day window."""
        return SlidingWindowCounter(window_size=WindowSizes.DAY_1, limit=limit)
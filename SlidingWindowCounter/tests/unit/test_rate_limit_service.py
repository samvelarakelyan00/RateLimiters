...
def test_raise_ip_limit_raises_http_exception() -> None:
    ...
    assert exc.value.detail == "Sliding Window Counter -> Your IP limit exceeded; please try again later!"
...
def test_raise_email_limit_raises_http_exception() -> None:
    ...
    assert exc.value.status_code == 429
    assert exc.value.detail == "Sliding Window Counter -> Your email limit exceeded; please try again later!"
...
def test_raise_custom_limit_exceeded() -> None:
    ...
    assert exc.value.status_code == 429
    assert exc.value.detail == "Sliding Window Counter -> Your API limit exceeded; please try again later!"
...
def test_raise_custom_limit_with_different_resource() -> None:
    ...
    assert exc.value.status_code == 429
    assert exc.value.detail == "Sliding Window Counter -> Your Download limit exceeded; please try again later!"
